...
b_hf_boost, a_hf_boost = shelving_high(fc=fc, G=10, fs=fs)
b_hf_cut, a_hf_cut = shelving_high(fc=fc, G=-10, fs=fs)


audio = np.fromfile("input/sweep_20_3k4.pcm", dtype=np.int16)
y_boost = lfilter(b_hf_boost, a_hf_boost, audio)
y_cut = lfilter(b_hf_cut, a_hf_cut, audio)
